...
export class CSSGen {
  ...
  static isStyleWindClass(classNameOrStyle) {
    return (
  ...
  staticPropertyClasses = Object.values(this.staticPropertyClassesRegEx)

  dynamicPropertyClassesRegEx = {
    MARGIN: /^m(.)?-(.*)/,
    PADDING: /^p(.)?-(.*)/,
    SCALE: /^scale-(.*)/,
    BORDER: /^border-[0-9]/,
  ...
  hydrateStaticClasses(className) {
  ...
  hydrateDynamicPropertyClasses(className) {
    const styledClassName = CSSGen.getClassName(className)
    ...
    if (dyanmicPopertyClass) {
      ...
      if (styledClassName.match(this.dynamicPropertyClassesRegEx.MARGIN)) {
        const [, direction, value] = styledClassName.match(dyanmicPopertyClass)
        ...
        const getBorderRadiusForDirection = (direction, radiusValue) => {
          switch (direction) {
            case 't': {
              return `
  ...
  static expandDirectionChar(direction) {
  ...
  hydrateNormalClasses(className) {
    ...
    if (CSSGen.isStyleWindClass(css)) {
      ...
      console.warn(`unknown class name ${css}`)
    }

    return css
  }

  hydratePseudoClasses(pseudoClass) {
    ...
    switch (prefix.trim()) {
      ...
      default: {
        console.warn(`unknown pseudo class ${prefix}`)
  ...
  static isPseudoClass(className) {
  ...
  static getPseudoAndClassName(pseudoClassName) {
  ...
  static usesSwindAPI(className) {
    const [keyword] = className.trim().split(':')

    return keyword.trim().match(SWIND_API_REGEX)
  }

  static usesDotNotationAPI(className) {
